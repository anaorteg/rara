...
for   j= 1:f_s(3)/2 %1st camera position
...
Zaxisprofile = Zaxisprofile(1 : f_s(3)/2);
...
figure;plot(Zaxisprofile); title('Zaxisprofile 1st camera position');xlabel('Projection number');
ylabel('Mean intensity)');
...
for  j = f_s(3)/2+1 : f_s(3) %2nd camera position
       %outI = mean_match(F(:,:,j), m);
    outI = F(:,:,j);
    CMean=mean(outI,1);
    VMean=mean(CMean,2);
...
Zaxisprofile = Zaxisprofile(f_s(3)/2+1 : f_s(3));
illumination_var = smooth(Zaxisprofile,'rlowess');
illumination_var =  illumination_var';
resp_sig = Zaxisprofile-illumination_var;
figure;plot(illumination_var);title(' illumination_var in 2nd camera position');xlabel('Projection number');
ylabel('Mean intensity)');

figure;plot(Zaxisprofile);title(' Zaxisprofile in 2nd camera position');xlabel('Projection number');
%% 
ylabel('Mean intensity)');
